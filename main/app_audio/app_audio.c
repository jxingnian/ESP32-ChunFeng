...
#include <string.h> /* 重复包含,建议删除 */
#include "esp_err.h" /* ESP32错误码定义 */
#include "freertos/FreeRTOS.h" /* FreeRTOS操作系统核心头文件 */
#include "esp_log.h" /* ESP32日志功能 */

/* GMF(Generic Media Framework)通用媒体框架相关头文件 */
#include "esp_gmf_element.h" /* GMF元素定义 */
#include "esp_gmf_pool.h" /* GMF内存池管理 */
#include "esp_gmf_io.h" /* GMF输入输出接口 */
#include "esp_gmf_pipeline.h" /* GMF管道处理 */
#include "esp_gmf_pool.h" /* GMF内存池管理(重复包含) */
#include "esp_audio_simple_player.h" /* 简单音频播放器 */
#include "esp_audio_simple_player_advance.h" /* 高级音频播放器功能 */
#include "esp_gmf_setup_pool.h" /* GMF内存池初始化 */
#include "esp_gmf_setup_peripheral.h" /* GMF外设初始化 */
#include "esp_gmf_gpio_config.h" /* GMF GPIO配置 */
#include "esp_codec_dev.h" /* 音频编解码器设备 */
#include "esp_gmf_fifo.h" /* GMF FIFO缓冲区 */

/* 根据是否启用按键对话模式包含不同头文件 */
#ifndef CONFIG_KEY_PRESS_DIALOG_MODE
#include "esp_vad.h" /* 语音活动检测(Voice Activity Detection) */
#include "esp_afe_config.h" /* 音频前端(Audio Front End)配置 */
#include "esp_gmf_afe_manager.h" /* AFE管理器 */
#include "esp_gmf_afe.h" /* AFE基础功能 */
#endif  /* CONFIG_KEY_PRESS_DIALOG_MODE */
#include "audio_processor.h" /* 音频处理器主要功能 */

/* 定义常量 */
#define VAD_ENABLE       (true)      // 是否启用VAD
...
static char *TAG = "AUDIO_PROCESSOR"; // 日志标签
...
typedef struct {
    ...
    esp_gmf_pool_handle_t  pool;     // GMF资源池句柄
} audio_manager_t;
...
esp_err_t audio_manager_init(void)
{
    ...
    // 注册各种资源到资源池
    pool_register_io(audio_manager.pool);                // 注册IO资源
...
esp_err_t audio_recorder_open(recorder_event_callback_t cb, void *ctx)
{
    ...
    audio_recorder.afe_cfg->vad_init = VAD_ENABLE;           // 启用语音活动检测
    audio_recorder.afe_cfg->vad_mode = VAD_MODE_3;          // VAD模式3
    audio_recorder.afe_cfg->vad_min_speech_ms = 64;         // 最小语音持续时间64ms
    audio_recorder.afe_cfg->vad_min_noise_ms = 1000;        // 最小噪声持续时间1000ms

#if CONFIG_VOICE_WAKEUP_MODE
    audio_recorder.afe_cfg->wakenet_init = true;            // 语音唤醒模式下启用wakenet
#else
    audio_recorder.afe_cfg->wakenet_init = false;           // 非语音唤醒模式下禁用wakenet
#endif  // CONFIG_VOICE_WAKEUP_MODE

    audio_recorder.afe_cfg->aec_init = true;                // 启用回声消除
    ...
    gmf_afe_cfg.vcmd_detect_en = VCMD_ENABLE;              // 启用语音命令检测
    gmf_afe_cfg.wakeup_end = 60000;                        // 唤醒超时时间60秒
    ...
    cfg.ctx = NULL;
    cfg.cb = NULL;
    cfg.thread.core = 0;                                    // 运行在核心0
    cfg.thread.prio = 5;                                   // 优先级5
    cfg.thread.stack = 5120;                               // 栈大小5KB
 ...
 * 
 * @return esp_err_t ESP_OK:成功 ESP_FAIL:失败
 * @note 播放固定格式的音频文件:16kHz采样率,单声道,16位采样深度
 */
esp_err_t audio_playback_run(void)
{
    ...
    music_info.sample_rate = 16000;  // 16kHz采样率
    music_info.channels = 1;         // 单声道
    music_info.bits = 16;           // 16位采样深度
    music_info.bitrate = 0;         // 比特率设为0
...
esp_err_t audio_prompt_open(void)
{
    ...
    esp_asp_cfg_t cfg = {
        .in.cb = NULL,
        .in.user_ctx = NULL,
        .out.cb = prompt_out_data_callback,  // 设置输出回调函数
        .out.user_ctx = NULL,
        .task_prio = 5,                      // 设置任务优先级
    };
    // 创建提示音播放器
